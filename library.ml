...
class enemy =
  object (self)
    ...
    method battle : unit =
      if health > 0 then (
        ANSITerminal.print_string [ red ] name;
        printf " attacks you for ";
        ANSITerminal.print_string [ red ] (string_of_int dmg);
        printf " damage\n";
        printf "What will you do? (";
        ANSITerminal.print_string [ yellow ] "attack";
        printf ")\n";
  ...
  object (self)
    ...
    method enter : unit =
      if String.length name > 0 then (
        printf "You enter ";
        ANSITerminal.print_string [ cyan ] name;
        printf "\n");
      List.iter (fun a -> printf "%s " a) messages;
      printf "\n";
      self#priority
    ...
    method priority : unit =
      if List.length enemies > 0 then (
        ...
        self#priority)
      else if List.length rooms > 0 then (
        List.iter
          (fun a ->
            printf "You can choose to enter ";
            ANSITerminal.print_string [ cyan ] a#name;
            printf "\n")
          rooms;
        self#choose_room)
  end
